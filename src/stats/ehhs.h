...
class EHHS_ehh_data_unphased: public EHHS_ehh_data{
    public:
    // double prev_ehh_before_norm = 0;
    // double curr_ehh_before_norm = 0;
...
class EHHS : public SelscanStats{
    public:
        EHHS(const std::unique_ptr<HapMap>&  hm, param_main& params) : SelscanStats(hm, params){}
        void main();
        ~EHHS(){}
    ...
    private:
        ...
        int max_extend;
        pair<double, double> calc_EHHS(int locus);
        ...
        void updateEHH_from_split(const unordered_map<int, vector<int> > & m, EHHS_ehh_data* p);
