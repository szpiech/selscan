...
void EHHS::updateEHH_from_split(const unordered_map<int, vector<int> > & m, EHHS_ehh_data* ehhdata){
    for (const auto &ele : m) {
        int old_group_id = ele.first;
        int newgroup_size = ele.second.size() ;
...
pair<double, double> EHHS::calc_ehh_unidirection_unphased(int locus,  bool downstream){
    int numSnps = hm->hapData->nloci; // must be same for both hapData and hapData2

    double ihh_p1 = 0;
    double ihh_p2 = 0;

    EHHS_ehh_data_unphased* p1, *p2, *pooled;
    p1 = new EHHS_ehh_data_unphased();
    p2 = new EHHS_ehh_data_unphased();
    pooled = new EHHS_ehh_data_unphased();
...
*/
pair<double, double> EHHS::calc_ehh_unidirection(int locus,  bool downstream){
    if(p.UNPHASED){
    ...
    EHHS_ehh_data* p1, *p2, *pooled;
    p1 = new EHHS_ehh_data();
    p2 = new EHHS_ehh_data();
    pooled = new EHHS_ehh_data();
    ...
    while(true){
        ...
        if(p1->totgc != p1->nhaps){
            ...
            ehh_p1 = p1->curr_ehh_before_norm/p1->normalizer;
        ...
        if(p2->totgc != p2->nhaps){
            ...
            ehh_p2 += p2->curr_ehh_before_norm /p2->normalizer;
            p2->prev_ehh_before_norm = p2->curr_ehh_before_norm;
            m.clear();
        }
    ...
    return make_pair(ehh_p1, ehh_p2);
}
...
void EHHS::main()
{
    ...
    (*fout) << "id\tpos\tgpos\tp1\tehh_p1\tp2\tehh_p2\n";
...
*/
pair<double, double> EHHS::calc_EHHS(int locus)
{
