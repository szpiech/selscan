...
// Quantile-bin aware: windows are grouped by SNP count (numQuantiles)  
// Extreme values (percentiles) are computed within each SNP-count bin  
// This normalizes for SNP density, so high/low signals are fair across windows
//Windows have different numbers of SNPs. To compare them fairly, we sort windows by SNP count 
//and split them into numQuantiles bins. Each window is then compared only to others in its bin, so low-SNP windows are judged with low-SNP windows, and high-SNP windows with high-SNP windows.
void analyzeBPWindows(const string normedfiles[], const int fileLoci[], int nfiles, int winSize, int numQuantiles, int minSNPs, StatType statType)
{
    ...
    // for xpehh
    double freq1, freq2;
    int numWindowsTop = 0, numWindowsBot = 0;
    ...
    if (statType == IHS || statType == IHH12)
    {
        ...
        */
        ...
        map<int, double> *topWindowBoundary = new map<int, double>[numQuantiles];
        ...
        for (int i = 0; i < numWindows; i++)
        {
            if (allSNPsPerWindow[i] <= quantileBound[b]) {
                ...
                for (int p = 1; p <= 100; p++) {
                    topWindowBoundary[b][p] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindow[start]), 1, count, 1 - double(p) / 100.0);
                }

                cerr << quantileBound[b] << " "
                     << topWindowBoundary[b][1] << " "
                     << topWindowBoundary[b][5] << endl;
        ...
        gsl_sort(&(allFracCritPerWindow[start]), 1, count);
        topWindowBoundary[b][1] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindow[start]), 1, count, 0.990);
        topWindowBoundary[b][5] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindow[start]), 1, count, 0.950);

        cerr << quantileBound[b] << " "
             << topWindowBoundary[b][1] << " "
             << topWindowBoundary[b][5] << "\n\n";
        ...
        for (int i = 0; i < nfiles; i++)
        {
            ...
            for (int j = 0; j < nSNPs[i].size(); j++)
            {
                ...
                // Find percentile 1–100
                int percentile = 100;
                for (int p = 1; p <= 100; p++) {
                    if (fracCrit[i][j] >= topWindowBoundary[b][p]) {
                        percentile = p;
                        break;
                    }
                }
    ...
    else if (statType == XPEHH)
    {
        ...
        // Allocate maps for 1–100 percentiles for each quantile bin
        map<int, double> *topWindowBoundaryTop = new map<int, double>[numQuantiles];
        ...
        for (int i = 0; i < numWindowsTop; i++)
        {
            ...
            else
            {
                ...
                for (int p = 1; p <= 100; p++)
                    topWindowBoundaryTop[bTop][p] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindowTop[startTop]), 1, countTop, 1-double(p)/100.0);
                    
                cerr << quantileBoundTop[bTop] << " "
                     << topWindowBoundaryTop[bTop][1] << " "
                     << topWindowBoundaryTop[bTop][5] << endl;
        ...
        gsl_sort(&(allFracCritPerWindowTop[startTop]), 1, countTop);
        topWindowBoundaryTop[bTop][1] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindowTop[startTop]), 1, countTop, 0.990);
        topWindowBoundaryTop[bTop][5] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindowTop[startTop]), 1, countTop, 0.950);

        cerr << quantileBoundTop[bTop] << " "
             << topWindowBoundaryTop[bTop][1] << " "
             << topWindowBoundaryTop[bTop][5] << "\n\n";
        ...
        map<int, double> *topWindowBoundaryBot = new map<int, double>[numQuantiles];
        ...
        for (int i = 0; i < numWindowsBot; i++)
        {
            ...
            else
            {
                ...
                for (int p = 1; p <= 100; p++) {
                    topWindowBoundaryBot[bBot][p] = gsl_stats_quantile_from_sorted_data(&(allFracCritPerWindowBot[startBot]), 1, countBot, 1 - double(p) / 100.0);
                }

                cerr << quantileBoundBot[bBot] << " "
                     << topWindowBoundaryBot[bBot][1] << " "
                     << topWindowBoundaryBot[bBot][5] << endl;
        ...
        for (int i = 0; i < nfiles; i++)
        {
            ...
            for (int j = 0; j < nSNPs[i].size(); j++)
            {
                ...
                // Percentiles for Top
                int percentileTop = 100;
                for (int p = 1; p <= 100; p++) {
                    if (fracCritTop[i][j] >= topWindowBoundaryTop[bTop][p]) {
                        percentileTop = p;
                        break;
                    }
                }

                // Percentiles for Bottom
                int percentileBot = 100;
                for (int p = 1; p <= 100; p++) {
                    if (fracCritBot[i][j] >= topWindowBoundaryBot[bBot][p]) {
                        percentileBot = p;
                        break;
                    }
                }
...
void getMeanVarBins(double freq[], double data[], int nloci, double mean[], double variance[], int n[], int numBins, double threshold[], string normLogInput)
{
    ...
    if(normLogInput!=""){
        for (int b = 0; b < numBins; b++)
        {
            //mean[b] = ; read from file
            //variance[b] ; read from file
        }
